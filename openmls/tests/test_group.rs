...
// This tests the unordered message processing of a message in a future generation.
#[test]
fn group_unordered_messages_future() {
    for ciphersuite in Config::supported_ciphersuites() {
        let group_aad = b"Alice's test group";
        ...
        // === Alice generates multiple messages ===
        let MAXIMUM_FORWARD_DISTANCE = 1000;
        let mut messages = Vec::new();
        for x in 0 .. MAXIMUM_FORWARD_DISTANCE+2 {
            let msg = group_alice
                .create_application_message(&[], &[1, 2, 3], &alice_credential_bundle, 0)
                .unwrap();
            messages.push(msg);
        }

        //  === Bob decrypts the last message generated by alice too distant in the future  ===
        let expected_error = match group_bob.decrypt(&messages.pop().unwrap()) {
            Ok(mls_plaintext) => panic!("Bob should not decrypt message too distant in the future"),
            Err(e) => e,
        };

        //  === Bob decrypts an unordered message by alice (but inside the distance margins)  ===

        let mls_plaintext_bob = match group_bob.decrypt(&messages.pop().unwrap()) {
            Ok(mls_plaintext) => mls_plaintext,
            Err(e) => panic!("Error decrypting MlsCiphertext: {:?}", e),
        };
    }
...
// This tests the unordered message processing using a future message and then processing the previous messages in a past generation.
// Checking that messages respect the boundaries for the past messages and that the ratchet can be used to decrypt out of order messages.
#[test]
fn group_unordered_messages_past() {
    for ciphersuite in Config::supported_ciphersuites() {
        let group_aad = b"Alice's test group";
        ...
        //  === Bob decrypts the last message generated by alice ===
        let mls_plaintext_bob = match group_bob.decrypt(&messages.pop().unwrap()) {
            Ok(mls_plaintext) => mls_plaintext,
            Err(e) => panic!("Error decrypting MlsCiphertext: {:?}", e),
        };

        let too_far_into_past_msg = messages.remove(0);
        let expected_error_past = match group_bob.decrypt(&too_far_into_past_msg) {
            Ok(mls_plaintext) => panic!("Bob should not be able to decrypt messages too far in the past"),
            Err(e) => e ,
        };

        //  === Bob decrypts all past messages  ===
        for msg in messages {
            let mls_plaintext_bob = match group_bob.decrypt(&msg) {
                Ok(mls_plaintext) => mls_plaintext,
                Err(e) => panic!("Error decrypting MlsCiphertext: {:?}", e),
            };
        }
...
>>>>>>> 658fb207... Fixes out-of-order messages. Issue #453.
